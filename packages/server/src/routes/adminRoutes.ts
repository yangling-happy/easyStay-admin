...
router.get("/hotels/published", async (req, res) => {
  try {
    const { location, keyword, rooms, guests, minPrice, maxPrice, stars, page, limit, startDate, endDate } = req.query;
    ...
    // 转换 _id 为 id，并对 roomTypes 按价格排序，应用动态定价
    const hotelsWithId = hotels.map((hotel) => {
      ...
      if (hotelObj.roomTypes && Array.isArray(hotelObj.roomTypes)) {
        ...
        // 应用动态定价机制
        (hotelObj as any).roomTypes = calculateDynamicPrices(hotelObj.roomTypes, String(startDate), String(endDate));
      }
      return {
        ...hotelObj,
