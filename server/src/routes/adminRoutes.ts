...
router.get('/hotels/pending', async (req, res) => {
  // 当有人访问 GET /api/admin/hotels/pending 时，执行这个函数
  try {
    // 1. 从数据库查询数据
    const hotels = await HotelModel.find({
      status: 'pending',      // 只找 status 是 'pending' 的酒店
      isDeleted: false        // 只找 isDeleted 是 false 的酒店（没被删除）
    }).sort({ createTime: -1 }); // 按创建时间倒序排列（最新的在前）
