 ...
 */
async function notifyAdminsOfPendingHotel(hotelId, hotelName) {
    try {
        const admins = await User.find({ role: "admin" }).select("_id");
        if (admins.length === 0)
            return;
        const message = `有新的酒店"${hotelName}"待审核`;
        await NotificationModel.insertMany(admins.map((a) => ({
            type: "pending_audit",
            hotelId,
            hotelName,
...
router.post("/", auth, async (req, res) => {
    try {
        const isIncomplete = req.body?.isIncomplete === true ||
...
router.put("/:id", auth, async (req, res) => {
    try {
        ...
        const beforeSnapshotWithoutId = { ...beforeSnapshot };
        ...
        const updatedHotel = await HotelModel.findOneAndUpdate({ _id: req.params.id, ownerId: req.user?.userId }, updateData, { new: true });
        if (!updatedHotel) {
            return res.status(404).json({ success: false, message: "未找到酒店" });
        }
        const auditEntry = {
        ...
        await HotelModel.updateOne({ _id: req.params.id }, { $push: { auditHistory: auditEntry } });
        if (shouldForcePending) {
            await notifyAdminsOfPendingHotel(String(updatedHotel._id), updatedHotel.name);
        }
        console.log("酒店更新成功:", {
            id: updatedHotel._id,
 ...
 * @description 下线酒店
 */
router.patch("/:id/offline", auth, async (req, res) => {
    try {
        const hotel = await HotelModel.findOne({
            _id: req.params.id,
            ownerId: req.user?.userId,
        ...
        const updatedHotel = await HotelModel.findOneAndUpdate({ _id: req.params.id, ownerId: req.user?.userId }, {
            isActive: false,
            status: "offline",
            updateTime: new Date(),
        }, { new: true });
 ...
 * @description 上线酒店
 */
router.patch("/:id/online", auth, async (req, res) => {
    try {
        const hotel = await HotelModel.findOne({
            _id: req.params.id,
            ownerId: req.user?.userId,
 ...
 * @description 重新上线酒店
 */
router.post("/:id/re-apply", auth, async (req, res) => {
    try {
        const hotel = await HotelModel.findOne({
            _id: req.params.id,
            ownerId: req.user?.userId,
...
router.post("/restore", auth, async (req, res) => {
    try {
        ...
        const updatedHotel = await HotelModel.findOneAndUpdate({ _id: id, ownerId: req.user?.userId, isDeleted: true }, {
            isDeleted: false,
            updateTime: new Date(),
        }, { new: true });
        if (!updatedHotel) {
        ...
        await HotelModel.updateOne({ _id: id }, { $push: { auditHistory: auditEntry } });
        res.json({ success: true, data: updatedHotel });
    }
    catch (error) {
        const errorMessage = error instanceof Error ? error.message : "未知错误";
...
router.delete("/:id", auth, async (req, res) => {
    try {
        ...
        await HotelModel.findOneAndUpdate({ _id: req.params.id, ownerId: req.user?.userId }, {
            isDeleted: true,
            updateTime: new Date(),
        });
    ...
    try {
        ...
        for (const hotel of hotels) {
            try {
                ...
                await HotelModel.updateOne({ _id: hotel._id, ownerId: hotel.ownerId }, {
                    isDeleted: true,
                    updateTime: new Date(),
                });
                successCount++;
