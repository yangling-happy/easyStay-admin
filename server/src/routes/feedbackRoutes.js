 ...
 */
async function notifyAdminsOfNewFeedback(feedbackId, hotelId, hotelName) {
    try {
...
router.post("/", async (req, res) => {
    try {
        ...
        await notifyAdminsOfNewFeedback(String(newFeedback._id), hotelId ? String(hotelId) : undefined, hotelName);
...
router.get("/list", async (req, res) => {
    try {
        ...
        const listWithHotelName = await Promise.all(list.map(async (item) => {
            let hotelName = "";
            let hotelNameEn = "";
            if (item.hotelId) {
                try {
...
router.patch("/:id/reply", async (req, res) => {
    try {
        ...
        const updated = await FeedbackModel.findByIdAndUpdate(id, {
            reply,
            status: "replied",
            repliedAt: new Date(),
        }, { new: true });
