...
router.get("/hotels/published", async (req, res) => {
    try {
        ...
        if (keyword) {
            ...
            const keywords = keywordStr.split(/\s+/).filter((k) => k.trim());
            if (keywords.length > 0) {
                const orConditions = [];
                keywords.forEach((k) => {
                    orConditions.push({ name: { $regex: k, $options: "i" } }, { nameEn: { $regex: k, $options: "i" } }, { amenities: { $regex: k, $options: "i" } });
                });
        ...
        if (rooms || guests) {
            ...
            hotels = hotels.filter((hotel) => {
                // 检查是否有足够的房间库存和容量
                return hotel.roomTypes.some((room) => {
                    return (room.stock >= requiredRooms &&
        ...
        const hotelsWithId = hotels.map((hotel) => {
            ...
            if (hotelObj.roomTypes && Array.isArray(hotelObj.roomTypes)) {
                hotelObj.roomTypes.sort((a, b) => a.price - b.price);
            }
            return {
                ...hotelObj,
...
router.post("/hotels/:id/audit", async (req, res) => {
    try {
        ...
        delete snapshotWithoutId._id;
        delete snapshotWithoutId.__v;
        delete snapshotWithoutId.auditHistory;
        const updateData = {
            status,
            updateTime: new Date(),
            $push: {
